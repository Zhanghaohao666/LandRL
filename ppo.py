...
from tools import ValueNorm, make_mlp, IndependentNormal, Actor, GAE, make_batch, IndependentBeta, BetaActor, vec_to_world, ResNet18FeatureExtractor, TransformerFeatureExtractor
...
class PPO(TensorDictModuleBase):
    def __init__(self, cfg, observation_spec, action_spec, device):
        ...
        # 用纯 MLP 提取特征（仅基于 state）
        self.feature_extractor = TensorDictSequential(
            TensorDictModule(make_mlp([256, 256]), [("agents", "observation", "state")], ["_feature"]),
        ).to(self.device)
        ...
        # Critic: 加一层隐藏层，提升 Value 估计精度
        # 原来只有一层 LazyLinear(1)，对 GAE 的 advantage 质量影响很大
        self.critic = TensorDictModule(
            nn.Sequential(nn.LazyLinear(128), nn.LeakyReLU(), nn.Linear(128, 1)),
    ...
    def __call__(self, tensordict):
        ...
        # 输出速度动作；若环境提供局部目标方向，则转换回世界系。
        actions = (2 * tensordict["agents", "action_normalized"] * self.cfg.actor.action_limit) - self.cfg.actor.action_limit
        direction = tensordict.get(("agents", "observation", "direction"), None)
        if direction is not None:
            actions = vec_to_world(actions, direction)
        tensordict["agents", "action"] = actions
        return tensordict

    def train(self, mode=True):
        # Preserve the standard nn.Module train/eval API so `policy.eval()`
        # does not accidentally enter the PPO update path.
    ...
    def update(self, tensordict):
        ...
        with torch.no_grad():
            ...
            next_tensordict = torch.vmap(self.feature_extractor, randomness="different")(next_tensordict)
    ...
    def _update(self, tensordict): # tensordict shape (batch_size, )
        ...
        # Feature extractor 同时接收 actor + critic 的梯度，也需要裁剪防止发散
