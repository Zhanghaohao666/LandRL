...
def main(cfg):
    ...
    vel_transform = AttitudeController(controller)
    ...
    for i, data in enumerate(collector):
        ...
        if done is None:
            term = data.get(("next", "terminated"), None)
            trunc = data.get(("next", "truncated"), None)
        ...
        if done is not None:
            ...
            print(f"[batch {i}] done_total = {total_done} / {done.numel()}")

        # 打印reach_goal统计信息
        if ("next", "stats", "reach_goal") in data.keys(True, True):
            Landed = data[("next", "stats", "reach_goal")]
            total_Landed = int(Landed.sum())
            print(f"[batch {i}] Landed_total = {total_Landed} / {Landed.numel()}")

        for_time = datetime.datetime.now()
        print("Time Elapsed: ", for_time - start_time)
        start_time = for_time
