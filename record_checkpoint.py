...
def resolve_sim_experience(headless: bool, log_video: bool = True) -> str:
    # stats-only 外部评估不需要渲染，使用 headless.kit 避免无 DISPLAY/Vulkan 环境下卡住。
    # 真正录视频时仍使用 rendering.kit。
    needs_rendering = (not headless) or bool(log_video)
    experience_name = (
        "isaaclab.python.headless.rendering.kit"
        if headless and needs_rendering
        else "isaaclab.python.headless.kit"
        if headless
...
def extract_first_env_trace(
    ...
    dz = first_env_obs[:, 1]
    # [简化观测适配] 新 6 维观测布局: [dx, dy, dz, vx, vy, vz]
    # dx=obs[:,0], dy=obs[:,1], dz=obs[:,2], vx=obs[:,3], vy=obs[:,4], vz=obs[:,5]
    obs_dim = first_env_obs.shape[-1]
    if obs_dim >= 12:
        # 旧 12 维布局兼容
        vx = first_env_obs[:, 6]
        vy = first_env_obs[:, 7]
        ...
        # 新 6 维布局: [dx, dy, dz, vx, vy, vz]
        dx = first_env_obs[:, 0]
        dy = first_env_obs[:, 1]
        horizontal_err = torch.sqrt(dx.square() + dy.square())
        dz = first_env_obs[:, 2]
        vx = first_env_obs[:, 3]
        vy = first_env_obs[:, 4]
        vz = first_env_obs[:, 5]
        # toward_speed / cross_speed 从 dx,dy,vx,vy 近似计算
        h_err_safe = horizontal_err.clamp_min(1e-6)
        rpos_nx = dx / h_err_safe
        rpos_ny = dy / h_err_safe
        toward_speed = vx * rpos_nx + vy * rpos_ny
        cross_speed = -vx * rpos_ny + vy * rpos_nx
...
def main():
    ...
    sim_experience = resolve_sim_experience(cfg.headless, log_video=log_video)
    ...
    try:
        ...
        if args.constant_motor_cmd is not None:
            ...
            try:
                ...
                log_record(f"[record][warn] env.set_seed({cfg.seed}) failed: {exc}")
            ...
            # 控制链路与 train.py 一致：策略(vx,vy,vz) → EMA平滑 → VelController(Lee) → 电机命令
            # Compose._inv_call 按逆序执行，vel_ema 放列表末尾 = _inv_call 时最先执行
            ...
            vel_ema = VelocityEMATransform(
                num_envs=env.num_envs, device=cfg.device, alpha=ema_alpha,
            )
            transforms.append(vel_ema)
            # 注入 EMA transform 引用到 env，确保 reset 时缓冲正确清零
            ...
            try:
                ...
                log_record(f"[record][warn] transformed_env.set_seed({cfg.seed}) failed: {exc}")
                torch.manual_seed(int(cfg.seed))
