...
def _run_external_recording(
    *,
    ...
    print(f"[NavRL]: start external recorder '{stem}'", flush=True)
    print(f"[NavRL]: recorder log -> {log_path}", flush=True)
...
def _poll_external_eval_jobs(run, pending_jobs: list[dict], best_state: dict):
    ...
    for job in pending_jobs:
        ...
        step = int(job["step"])
        print(
            f"[NavRL]: async external recorder '{prefix}_{step}' finished with return code {return_code}",
            flush=True,
        )
        ...
        if return_code == 0 and os.path.exists(job["stats_path"]):
            ...
            print(
                f"[NavRL][warn]: async external recorder '{prefix}_{step}' did not produce usable stats. "
...
def _maybe_save_best_checkpoint(
    ...
    for metric_prefix in ("eval_suite/stats",):
        ...
        if success_count is not None and num_cases is not None:
            print(
                f"[NavRL]: best checkpoint updated at step {step} "
    ...
    try:
        ...
        if suite_enabled and suite_num_envs > 1:
            ...
            if suite_async and not allow_async_suite:
                print(
                    f"[NavRL][warn]: skipping async eval_suite_{step} because a previous suite job is still running.",
                    flush=True,
                )
...
def resolve_sim_experience(cfg) -> str:
    ...
    if os.path.isfile(experience_path):
        print(f"[NavRL]: loading Isaac Lab experience: {experience_path}")
        return experience_path

    print(
        "[NavRL][warn]: Isaac Lab experience file not found, fallback to SimulationApp default experience: "
        f"{experience_path}"
    )
    return ""
...
def configure_gpu_pipeline_compatibility(cfg):
    ...
    print(f"[NavRL]: Isaac Lab render flags applied. offscreen={bool(cfg.headless)}")
    ...
    print("[NavRL]: GPU pipeline compatibility settings applied for Isaac Lab rendering.")
...
def enable_required_isaac_extensions(sim_app):
    ...
    for extension_name in required_extensions:
        ...
        print(f"[NavRL]: enable_extension('{extension_name}') -> {enabled}")
...
def main(cfg):
    ...
    for i, data in enumerate(collector):
        ...
        if (i == 1) or (i > 0 and i % cfg.eval_interval == 0):
            ...
            try:
                ...
                print(f"\n[NavRL]: evaluation failed (non-fatal): {e}")
                print("[NavRL]: continuing training...")
            finally:
    ...
    if pending_external_jobs:
        print(
            f"[NavRL]: waiting for {len(pending_external_jobs)} pending external eval job(s) to finish...",
            flush=True,
        )
